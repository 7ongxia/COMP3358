...
def upload_file():
    ...
    if img_path:
        # Verify the selected file has the expected file type
        ...
        if file_extension not in ('.jpg', '.jpeg', '.png', '.gif', '.bmp'):
            ...
            resp = inbox_sqs.send_message(
                QueueUrl=queue_url, MessageBody=key, MessageDeduplicationId=key, MessageGroupId=key)
            print(resp)
    else:
